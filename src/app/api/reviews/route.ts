...
export async function GET() {
  try {
    ...
    let response = await fetch(
      `${SUPABASE_URL}/rest/v1/evaluations?featured=eq.true&feedback=not.is.null&select=id,rating_overall,feedback,created_at,customers(name,city)&order=created_at.desc&limit=6`,
      {
        headers: {
    ...
    // If no featured reviews, fall back to reviews with rating >= 4
